...
export const lockTokens = async (provider, bridgeContract, tokenContract, amount) => {
    ...
    let receipt = await bridgeContract.connect(provider.getSigner()).lock(await provider.getSigner().getAddress(), tokenContract.address, amount, result.deadline, result.v, result.r, result.s);
    await receipt.wait(1);
    return receipt
}
export const burnTokens = async (provider, bridgeContract, tokenContract, amount) => {
    console.log(bridgeContract.address)
    const burnMessageHash = ethers.utils.solidityKeccak256(["address", "address", "address", "uint256"], [await provider.getSigner().getAddress(), bridgeContract.address, tokenContract.address, amount]);
    ...
    let receipt = await bridgeContract.connect(provider.getSigner()).burn(await provider.getSigner().getAddress(), tokenContract.address, amount, burnSig.v, burnSig.r, burnSig.s);
    await receipt.wait(1);
    return receipt
}
...
export const claimTokens = async (provider, bridgeContract, lockTransaction) => {
      ...
      let sigs: Signature[] = [sig, sig2];
        ...
        let receipt = await bridgeContract
          .connect(provider.getSigner())
          .claimTokens(lockTransaction.tokenAddress, lockTransaction.amount, lockTransaction.lockTransactionHash, [sigs[0].v, sigs[1].v], [sigs[0].r, sigs[1].r], [sigs[0].s, sigs[1].s]);
        await receipt.wait(1);
